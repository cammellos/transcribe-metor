Transcribe.MusicXML.Part = function(xml) {
   this.xml = xml;
   this.id = xml.getAttribute('id');
   this.measures = [];
   // parse attributes as well, as they don't repeat in measures
   if (xml.getElementsByTagName('attributes').length) {
     this.attributes = new Transcribe.MusicXML.MeasureAttributes(xml.getElementsByTagName('attributes')[0]);
   } 
...
Transcribe.MusicXML.Part.prototype = {
   parseMeasures: function() {
     var measuresXML = this.xml.getElementsByTagName("measure");
     for(var i = 0; i < measuresXML.length; i++) {
        this.measures.push(new Transcribe.MusicXML.Measure(measuresXML[i],this.attributes));
     };
   },
